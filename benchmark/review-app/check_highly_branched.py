...
cursor.execute('''
    SELECT id, smiles, pfasgroups_detected, atlas_detected, 
           pfasgroups_success, atlas_success 
    FROM molecules 
...
for row in cursor.fetchall():
    ...
    print(f'  PFASGroups: success={pfas_success}, detected={len(pfas_groups)} groups: {pfas_groups[:5]}')
...
cursor.execute('''
    SELECT 
        COUNT(*) as total,
        SUM(CASE WHEN pfasgroups_success = 1 THEN 1 ELSE 0 END) as pfas_success,
        SUM(CASE WHEN atlas_success = 1 THEN 1 ELSE 0 END) as atlas_success,
        SUM(CASE WHEN json_array_length(pfasgroups_detected) > 0 THEN 1 ELSE 0 END) as pfas_detected,
...
print(f'PFASGroups success: {stats[1]}')
print(f'Atlas success: {stats[2]}')
print(f'PFASGroups detected groups: {stats[3]}')
