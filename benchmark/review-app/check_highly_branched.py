...
cursor.execute('''
    SELECT id, smiles, PFASGroupstected, atlas_detected, 
           PFASGroupsccess, atlas_success 
    FROM molecules 
...
for row in cursor.fetchall():
    ...
    print(f'  PFASGroupsuccess={pfas_success}, detected={len(pfas_groups)} groups: {pfas_groups[:5]}')
...
cursor.execute('''
    SELECT 
        COUNT(*) as total,
        SUM(CASE WHEN PFASGroupsccess = 1 THEN 1 ELSE 0 END) as pfas_success,
        SUM(CASE WHEN atlas_success = 1 THEN 1 ELSE 0 END) as atlas_success,
        SUM(CASE WHEN json_array_length(PFASGroupstected) > 0 THEN 1 ELSE 0 END) as pfas_detected,
...
print(f'PFASGroupsccess: {stats[1]}')
print(f'Atlas success: {stats[2]}')
print(f'PFASGroupstected groups: {stats[3]}')
