...
COLOR_YAML = SCRIPT_DIR.resolve().parents[2] / "PFASGroups" / "data" / "color_scheme.yaml"
...
def _make_tone_palette(base_colors: list, levels: list = _TONE_LEVELS) -> list:
    """Return tonal variants interleaved for maximum contrast between neighbours.

    Ordering: [c0_level0, c1_level0, ..., cN_level0, c0_level1, ...]
    so that consecutive feature sets use different base hues.
    """
    per_color: list[list[str]] = []
    for hex_color in base_colors:
        rgb = np.array(mcolors.to_rgb(hex_color))
        tones: list[str] = []
        for blend, target in levels:
            if target == "white":
                tone = rgb * blend + np.ones(3) * (1.0 - blend)
            elif target == "black":
                tone = rgb * blend
            else:
                tone = rgb.copy()
            tones.append(mcolors.to_hex(tuple(np.clip(tone, 0.0, 1.0).tolist())))
        per_color.append(tones)

    n_c = len(base_colors)
    result: list[str] = []
    for level_idx in range(len(levels)):
        for ci in range(n_c):
            result.append(per_color[ci][level_idx])
    return result


TONE_PALETTE = _make_tone_palette(PALETTE)
...
def _bar_style(idx: int) -> dict:
    """Return colour for bar at rank *idx*, cycling through the tone palette."""
    return dict(
        color=TONE_PALETTE[idx % len(TONE_PALETTE)],
        edgecolor="#333333",
        linewidth=0.4,
    )
