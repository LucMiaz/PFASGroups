...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Classify clinventory F-molecules with PFASGroups (halogens='F')"
    )
    ...
    p.add_argument("--all-halogens", action="store_true",
                   help="Fetch all halogenated molecules (not just F-containing ones)")
...
def fetch_molecules(conn, all_halogens: bool, limit: Optional[int], offset: int,
                    batch_size: int) -> List[Tuple[int, str, Optional[str]]]:
    """Return list of (id, smiles, formula) tuples.

    By default fetches only fluorine-containing molecules; pass
    ``all_halogens=True`` to include Cl/Br/I-only molecules as well.
    """
    if all_halogens:
        where = (
        ...
        # Fluorine-only: formula contains 'F' (possibly with digit suffix)
        where = (
            "WHERE smiles IS NOT NULL AND smiles != '' "
            "AND formula IS NOT NULL "
            "AND formula ~ 'F[0-9]*'"
    ...
    query = (
        f"SELECT id, smiles, formula FROM molecules {where} "
        f"ORDER BY id {limit_clause} {offset_clause}"
    )
...
def main():
    ...
    print("=" * 70)
    print("PFASGroups (F-only) — clinventory classification benchmark")
    print("=" * 70)
    print(f"  DB   : {args.db_user}@{args.db_host}:{args.db_port}/{args.db_name}")
    print(f"  Mode : {'all halogens' if args.all_halogens else 'fluorine-only (halogens=F)'}")
    ...
    rows = fetch_molecules(
        conn, args.all_halogens, args.limit, args.offset, args.batch_size
    )
    conn.close()
    ...
    for i, (mol_id, smiles, formula) in enumerate(rows):
        ...
        if (i + 1) % 500 == 0 or (i + 1) == n_total:
            ...
            print(
                f"  {i+1:>7,}/{n_total:,}  ({rate:,.0f} mol/s)  errors so far: {errors}",
                end="\r",
            )

    t_class_total = time.perf_counter() - t_class_start
    print(
        f"\n  Done in {t_class_total:.1f}s  "
        f"({n_total / t_class_total:.0f} mol/s)  errors: {errors}"
    )
    ...
    # Timing by halogen class (F only / mixed / other / none)
    ...
    summary = {
        ...
        "timestamp": timestamp,
        "db": {
            "name": args.db_name, "host": args.db_host,
    ...
    out_path = args.output or (
        data_dir / f"pfasgroups_clinventory_{timestamp}.json"
    )
    ...
    print(f"  Classified (any group): {len(classified):,}  "
          f"({100*len(classified)/max(len(valid),1):.1f}%)")
    print(f"  Has F-containing group: {len(f_group):,}  "
          f"({100*len(f_group)/max(len(valid),1):.1f}%)")
    if times:
        s = stats(times)
