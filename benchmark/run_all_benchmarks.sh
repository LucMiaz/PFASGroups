...
if [ "$REUSE_TIMING" = true ]; then
    echo "Reusing previous timing results mode enabled"
fi
...
if [ ! -f "scripts/classify/enhanced_pfas_benchmark.py" ]; then
    echo "❌ Error: scripts/classify/enhanced_pfas_benchmark.py not found!"
...
echo "1" | python scripts/classify/enhanced_pfas_benchmark.py
if [ $? -ne 0 ]; then
...
echo "2" | python scripts/classify/enhanced_pfas_benchmark.py
if [ $? -ne 0 ]; then
...
if [ "$REUSE_TIMING" = true ]; then
    echo "Reusing previous timing results..."
    printf "3\ny\n" | env PFAS_BENCH_TIMING_PROFILE=full python scripts/classify/enhanced_pfas_benchmark.py
else
    printf "3\nn\n" | env PFAS_BENCH_TIMING_PROFILE=full python scripts/classify/enhanced_pfas_benchmark.py
fi
if [ $? -ne 0 ]; then
...
if [ "$REUSE_TIMING" = true ]; then
    echo "Reusing previous timing results..."
    printf "3\ny\n" | env PFAS_BENCH_TIMING_PROFILE=no_resistance python scripts/classify/enhanced_pfas_benchmark.py
else
    printf "3\nn\n" | env PFAS_BENCH_TIMING_PROFILE=no_resistance python scripts/classify/enhanced_pfas_benchmark.py
fi
if [ $? -ne 0 ]; then
...
if [ "$REUSE_TIMING" = true ]; then
    echo "Reusing previous timing results..."
    printf "3\ny\n" | env PFAS_BENCH_TIMING_PROFILE=no_metrics python scripts/classify/enhanced_pfas_benchmark.py
else
    printf "3\nn\n" | env PFAS_BENCH_TIMING_PROFILE=no_metrics python scripts/classify/enhanced_pfas_benchmark.py
fi
if [ $? -ne 0 ]; then
...
python scripts/analysis/compare_timing_profiles.py
if [ $? -ne 0 ]; then
...
echo "4" | python scripts/classify/enhanced_pfas_benchmark.py
if [ $? -ne 0 ]; then
...
echo "5" | python scripts/classify/enhanced_pfas_benchmark.py
if [ $? -ne 0 ]; then
...
python scripts/analysis/analyze_definitions_benchmark.py
if [ $? -ne 0 ]; then
...
echo "Adding test metadata to PFAS groups and definitions..."
if [ -f "scripts/data/add_test_metadata.py" ]; then
...
# Timing Analysis
echo "⏱Analyzing timing performance..."
TIMING_FILE=$(ls data/pfas_timing_benchmark_*.json 2>/dev/null | tail -1)
if [ -f "$TIMING_FILE" ]; then
    python scripts/analysis/analyze_timing.py "$TIMING_FILE"
    if [ $? -eq 0 ]; then
...
if [ -f "$DEFINITION_FILE" ]; then
    python scripts/analysis/analyze_definitions_benchmark.py "$DEFINITION_FILE"
    if [ $? -eq 0 ]; then
...
if [ -f "$COMPLEX_FILE" ]; then
    if [ -f "scripts/analysis/analyze_complex.py" ]; then
        python scripts/analysis/analyze_complex.py "$COMPLEX_FILE"
        if [ $? -eq 0 ]; then
...
if [ -f "$ENHANCED_FILE" ] && [ -f "$OECD_FILE" ]; then
    python scripts/analysis/enhanced_analysis.py "$ENHANCED_FILE" "$OECD_FILE"
    if [ $? -eq 0 ]; then
...
if [ -f "scripts/analysis/analyze_benchmarks_simple.py" ]; then
...
if [ -f "scripts/reports/generate_latex_tables.py" ]; then
