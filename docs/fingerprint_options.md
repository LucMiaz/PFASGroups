...
| per group | `min_dist_to_center` | $N_G$ | int ≥ 0 | $\min_{v \in S,\, c \in \mathrm{center}(G)} d(v,c)$ | yes | Topological proximity of the SMARTS-matched atoms to the molecular graph center. Zero when the functional group is at the center; large when it is a chain terminus. |
...
| per group | `min_dist_to_barycenter` | $N_G$ | int ≥ 0 | $\min_{v \in S,\, b \in \mathrm{bary}(G)} d(v,b)$, bary = node(s) minimising $\sum_u d(v,u)$ | yes | Distance of the functional group to the topological centre of mass. Complements `min_dist_to_center` for asymmetric chains and multi-functional molecules. |
