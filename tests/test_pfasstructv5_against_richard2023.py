...
SDF_PATH = Path(__file__).parent / "test_data" / "PFASSTRUCTV5_20221101.sdf"
...
if __name__ == "__main__":
    ...
    for i, (mol, name) in enumerate(_ALL_MOLECULES, start=1):
        if not _DEFINITION.applies_to_molecule(mol):
            ...
            f_ratio = _fluorine_ratio(mol, include_hydrogen=_DEFINITION.includeHydrogen)
            failures.append(
                {
                    "DTXSID": dtxsid,
                    "SMILES": smiles,
                    "F_ratio": round(f_ratio, 6),
                }
            )
        if i % 500 == 0:
    ...
    if failures:
        out_csv = SDF_PATH.parent / "pfasstructv5_unmatched.csv"
        with open(out_csv, "w", newline="") as fh:
            writer = csv.DictWriter(fh, fieldnames=["DTXSID", "SMILES", "F_ratio"])
            writer.writeheader()
